...
function extractMarketCap() {
  const parentDiv = document.querySelector(
    "div.text-xs.text-green-300.flex.w-full.justify-between.items-center"
  );
  if (parentDiv) {
    ...
    for (const div of childDivs) {
      if (div.textContent.includes("Market cap:")) {
        ...
        const marketCapMatch = marketCapText.match(
          /Market cap: \$(\d+,\d+\.\d+)/
        );
        if (marketCapMatch) {
...
function checkAndTriggerFireworks(marketCap) {
  if (
    marketCap &&
    marketCap > config.marketCapThreshold &&
    (marketCap >= lastMarketCapForFireworks + config.marketCapThreshold ||
      lastMarketCapForFireworks === 0)
  ) {
    const currentTimestamp = Date.now();
    if (
      currentTimestamp - lastCelebrationTimestamp >
      config.celebrationCooldown
    ) {
...
function processTransaction(transactionElement) {
  const transactionLink = transactionElement.querySelector(
  ...
  if (transactionLink) {
    ...
    if (transactionId !== lastTransactionId) {
      ...
      const usernameElement = transactionElement.querySelector(
        "div.py-3.pl-2.text-left.flex.items-center.flex-wrap button span span"
      );
      const actionElement = transactionElement.querySelector(
        "div.p-3.text-left.text-green-300.hidden.sm\\:block"
      );
      const amountElements = transactionElement.querySelectorAll(
        "div.p-3.text-left.overflow-hidden.whitespace-nowrap"
      );
...
function captureMostRecentTransaction() {
  const transactionContainer = document.querySelector(
    "div.w-full.text-xs.sm\\:text-sm.text-gray-400.bg-transparent.rounded-lg"
  );
  if (transactionContainer) {
    const transactionElements = transactionContainer.querySelectorAll(
      "div.text-xs.my-1.bg-\\[\\#2e303a\\].rounded-lg.grid.grid-cols-4.sm\\:grid-cols-6.items-start"
    );
...
function clickLoadTransactionsButton() {
  const button = document.querySelector(
    "div.cursor-pointer.px-1.rounded.hover\\:bg-gray-800.text-gray-500"
  ); // Replace with the actual selector for the button
  if (button) {
    button.click();
